...
class Solution:
    ...
    def get_solution(self):
        ...
        self.min_path_val = min(self.min_score_arr[self.end_pos[0]][self.end_pos[1]])
        # print(self.min_path_val)

        idx_in_path = self.get_idx_in_path(self.start_pos, 0)
        # print(sorted(idx_in_path))
        return len(idx_in_path)

    def get_idx_in_path(self, pos, direction_idx):
        if pos == self.end_pos:
            if self.min_score_arr[self.end_pos[0]][self.end_pos[1]][direction_idx] == self.min_path_val:
                return set([pos])
            else:
                return set()
        ...
        if self.maze[pos[0]][pos[1]] == '#':
            return set() # dead end
        # print(pos, direction_idx)
        
        consider = [
            ((pos[0] + self.directions[direction_idx][0], pos[1] +
                self.directions[direction_idx][1]), direction_idx, 1),
            ((pos[0] + self.directions[(direction_idx + 1) % 4][0], pos[1] +
                self.directions[(direction_idx + 1) % 4][1]), (direction_idx + 1) % 4, 1001),
            ((pos[0] + self.directions[(direction_idx - 1) % 4][0], pos[1] +
                self.directions[(direction_idx - 1) % 4][1]), (direction_idx - 1) % 4, 1001),
        ]
        idx_set = set()
        for new_pos, d_idx, score_offset in consider:
            if self.min_score_arr[pos[0]][pos[1]][direction_idx] + score_offset <= self.min_score_arr[new_pos[0]][new_pos[1]][d_idx]:
                idx_set = idx_set.union(self.get_idx_in_path(new_pos, d_idx))
        # print(pos, direction_idx, idx_set)
        if len(idx_set) == 0:
            return set() # dead end
        else:
            return idx_set.union(set([pos]))
