...
class Solution:
    ...
    def get_num_energized_tiles(self, start_row_pos, start_col_pos, start_row_vel, start_col_vel):
        # init
        self.energized = []
        self.next_beam_id = 1
        # dict to store beams: beam id
        self.beam_dict: dict[int, tuple[int, int, int, int]] = {0: (start_row_pos, start_col_pos, start_row_vel, start_col_vel)}
        # row_vel: number of rows per turn (ie "y vel")
        # col_vel: number of cols per turn (ie "x vel")
        self.visited: set[tuple[int, int, int, int]] = set([])
    ...
    def get_solution(self):
        ...
        self.num_rows, self.num_cols = len(self.grid), len(self.grid[0])
        configs = \
            [(row_idx, 0, 0, 1) for row_idx in range(self.num_rows)] + \
            [(row_idx, self.num_cols-1, 0, -1) for row_idx in range(self.num_rows)] + \
            [(0, col_idx, 1, 0) for col_idx in range(self.num_cols)] + \
            [(self.num_rows-1, col_idx, -1, 0) for col_idx in range(self.num_cols)]

        maximum = 0
        for tup in configs:
            maximum = max(maximum, self.get_num_energized_tiles(*tup))
