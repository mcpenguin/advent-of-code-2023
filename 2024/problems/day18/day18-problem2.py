...
class Solution:
    ...
    def process_line(self, line: str):
        ...
        self.corrupted_l.append((y, x)) # x, y
        # self.it += 1
    ...
    def get_solution(self):
        ...
        start = 1024
        end = len(self.corrupted_l)
        last_inf = 0
        while True:
            num_consider = (start + end) // 2
            print(num_consider)
            if abs(start - end) <= 1:
                break

            self.corrupted = set(self.corrupted_l[:num_consider])
            self.min_vals = {}
            for i in range(self.num_rows):
                for j in range(self.num_cols):
            ...
            print(self.min_vals[(self.num_cols-1), (self.num_rows-1)])
            print()
            if self.min_vals[(self.num_cols-1), (self.num_rows-1)] == math.inf:
                end = num_consider
                last_inf = num_consider
            else:
                start = num_consider

        return self.corrupted_l[last_inf-1]
